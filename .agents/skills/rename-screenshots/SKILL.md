...
description: Rename screenshots with descriptive names based on what each image shows.
---
# Rename Screenshots by Content

I have screenshots with generic timestamp names (e.g. `Screenshot 2025-09-03 at 14.13.34.png`).
Your job is to rename every file with a descriptive name based on what the screenshot shows.
...
2. Use subagents to view and rename files one by one.
   ...
   - Each subagent should process its batch sequentially (view an image, then rename it, view another, rename it) so image tokens are fresher in the context.
